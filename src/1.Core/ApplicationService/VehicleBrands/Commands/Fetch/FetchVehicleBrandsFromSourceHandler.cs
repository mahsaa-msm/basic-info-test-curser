...
public sealed class FetchVehicleBrandsFromSourceHandler : CommandHandler<FetchVehicleBrandsFromSourceCommand>
{
    ...
    public FetchVehicleBrandsFromSourceHandler(ZaminServices zaminServices,
    ...
    public override async Task<CommandResult> Handle(FetchVehicleBrandsFromSourceCommand command)
    {
        ...
        foreach (var item in response.Value!.DistinctBy(x => x.BrandID))
        {
            ...
            var updateParameter = new UpdateVehicleBrandParameter(title, DIPTitle.FromString(rawTitle), existing.Priority);
